...
public class MemberShip {
    // 多播组地址
    public static SocketAddress multicast_addr = new InetSocketAddress("228.5.6.7", 54321);
    ...
    class MemberSender extends TimerTask {
        ...
        public void run() {
            ...
            try {
                DatagramPacket pack = new DatagramPacket(data, data.length, multicast_addr);
                _sock.send(pack);
    ...
    class MemberReceiver extends TimerTask {
        ...
        public void run() {
            ...
            try {
                _sock.receive(pack);
                long time = Long.valueOf(new String(pack.getData(), 0, pack.getLength()));
                if (pack.getSocketAddress().equals(sock.getRemoteSocketAddress())
                        || pack.getSocketAddress().equals(sock.getLocalSocketAddress())) {
