	...
	public void process() {
		for (int i = 0; i < num; i++) {
			String name = WORKERNAME + "[" + (ranges[i] + 1) + "-"
					+ ranges[i + 1] + "]";
			new Thread(new LogMessageWorker(ranges[i] + 1, ranges[i + 1]), name)
					.start();
		}
	}

	public void getLogMsg(SVNRepository repository, long revision,
			LogMessageHandler handler) {
		try {
			SVNProperties properties = repository.getRevisionProperties(
					revision, null);
